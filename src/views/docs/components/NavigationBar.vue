<script setup>
import asideLinks from '@/components/ComponentList.vue'
</script>
<template>
    <div class="rows">
        <div class="row">
            <h1 id="description">Links</h1>
            <p>A link connects users to a different page or further information.</p>
            <details open>
                <summary>Component preview</summary>
                <div class="details-content">
                    <nav class="docs">
                        <ul>
                            <li><a href="#">Navigation link</a></li>
                            <li><a href="#">Navigation link</a></li>
                            <li><a href="#">Navigation link</a></li>
                            <li>
                                <a href="#" class="nav-item dropdown">
                                    Dropdown
                                    <ul>
                                        <li>
                                        <RouterLink to="#">Lorem ipsum dolor</RouterLink>
                                        </li>
                                    </ul>
                                </a>
                            </li>
                            <li>
                                <span href="#" class="nav-item dropdown">
                                    Dropdown
                                    <ul>
                                        <li>
                                        <RouterLink to="#">Home</RouterLink>
                                        </li>
                                        <li>
                                        <RouterLink to="#">About</RouterLink>
                                        </li>
                                    </ul>
                                </span>
                            </li>
                        </ul>
                        <button class="menu-collapse" v-on:click="updateHidden"><i class="bi bi-list"></i></button>
                    </nav>
                </div>
            </details>
            <div class="alert slim information">
                <p>The <code>&lt;a></code> can be used inside paragraphs, span etc.</p>
            </div>
        </div>
        <div class="row">
            <details>
                <summary>Component code</summary>
                <div class="details-content">
                    <div class="code-block">
                        <pre><code>&lt;nav&gt;
    &lt;ul&gt;
        &lt;li&gt;&lt;a href="#"&gt;Navigation link&lt;/a&gt;&lt;/li&gt;
        &lt;li&gt;&lt;a href="#"&gt;Navigation link&lt;/a&gt;&lt;/li&gt;
        &lt;li&gt;&lt;a href="#"&gt;Navigation link&lt;/a&gt;&lt;/li&gt;
        &lt;li&gt;
            &lt;a href="#" class="nav-item dropdown"&gt;
                Dropdown
                &lt;ul&gt;
                    &lt;li&gt;
                    &lt;a to="#"&gt;Lorem ipsum dolor&lt;/a&gt;
                    &lt;/li&gt;
                &lt;/ul&gt;
            &lt;/a&gt;
        &lt;/li&gt;
        &lt;li&gt;
            &lt;span href="#" class="nav-item dropdown"&gt;
                Dropdown
                &lt;ul&gt;
                    &lt;li&gt;
                    &lt;a to="#"&gt;Home&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                    &lt;a to="#"&gt;About&lt;/a&gt;
                    &lt;/li&gt;
                &lt;/ul&gt;
            &lt;/span&gt;
        &lt;/li&gt;
    &lt;/ul&gt;
    &lt;button class="menu-collapse" v-on:click="updateHidden"&gt;&lt;i class="bi bi-list"&gt;&lt;/i&gt;&lt;/button&gt;
&lt;/nav&gt;</code></pre>
                    </div>
                </div>
            </details>
            <p></p>
            <details>
                <summary>Component code (Menu collapse)</summary>
                <div class="details-content">
                    <div class="code-block">
                    <pre><code>export default {
    data() {
        return {
            left: "-70vw",
            isHidden: true,
        }
    },
    methods: {
        updateHidden() {
            this.isHidden = !this.isHidden;
            if(this.isHidden === true) {
                this.left = "-70vw";
            } else {
                this.left = 0;
            }
        },
    }
}</code></pre>
                </div>
                </div>
            </details>
            <p></p>
            <details>
                <summary>Component code (JavaScript version)</summary>
                <div class="details-content">
                    <div class="code-block">
                        <pre><code>&lt;!DOCTYPE html&gt;
&lt;html lang="en"&gt;
&lt;head&gt;
    &lt;meta charset="UTF-8"&gt;
    &lt;meta name="viewport" content="width=device-width, initial-scale=1.0"&gt;
    &lt;title&gt;Navbar Toggle&lt;/title&gt;
    &lt;style&gt;
        #navbar {
            position: fixed;
            top: 0;
            left: -70vw;
            width: 70vw;
            height: 100%;
            background-color: #333;
            color: white;
            transition: left 3000ms ease-in-out;
        }

        #toggleButton {
            position: fixed;
            top: 10px;
            right: 10px;
            z-index: 100;
            padding: 10px;
            background-color: #007BFF;
            color: white;
            border: none;
            cursor: pointer;
        }

        @media (min-width: 768px) {
            #toggleButton {
                display: none;
            }
        }
    &lt;/style&gt;
&lt;/head&gt;
&lt;body&gt;
    &lt;div id="navbar"&gt;
        &lt;p&gt;Navbar content here&lt;/p&gt;
    &lt;/div&gt;
    &lt;button id="toggleButton"&gt;Toggle Navbar&lt;/button&gt;

    &lt;script&gt;
        (function () {
            const navbar = document.getElementById('navbar');
            const toggleButton = document.getElementById('toggleButton');
            let isHidden = true;

            toggleButton.addEventListener('click', function () {
                isHidden = !isHidden;
                navbar.style.left = isHidden ? '-70vw' : '0';
            });
        })();
    &lt;/script&gt;
&lt;/body&gt;</code></pre>
                    </div>
                </div>
            </details>
        </div>
    </div>
    <aside>
        <asideLinks />
    </aside>
</template>

<style>
nav.docs {
    width: 100% !important;
    z-index: 0 !important;
}
</style>